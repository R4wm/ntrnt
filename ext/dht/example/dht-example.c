...
const unsigned char hash[20] = { 0x54, 0x57, 0x87, 0x89, 0xdf, 0xc4, 0x23, 0xee, 0xf6, 0x03, 0x1f, 0x81, 0x94, 0xa9,
...
int main(int argc, char **argv)
{
        ...
        while (1)
        {
                ...
                {
                        buf[rc] = '\0';
                        rc = dht_periodic(buf, rc, (struct sockaddr*) &from, fromlen, &tosleep, callback, NULL);
                } else
                {
        ...
        dht_uninit();
        return 0;
        usage: printf("Usage: dht-example [-q] [-4] [-6] [-i filename] [-b address]...\n"
                        "                   port [address port]...\n");
        exit(1);
}
